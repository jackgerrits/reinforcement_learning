...
namespace reinforcement_learning {
  ...
  int model_create<m::vw_model>(m::i_model** retval, const u::configuration& c, i_trace* trace_logger, api_status* status)
  {
    ...
    std::string initial_command_line = c.get(name::MODEL_VW_INITIAL_COMMAND_LINE, "--ccb_explore_adf --json --quiet --epsilon 0.0 --first_only --id N/A");
    const std::string sample_mode = c.get(name::CCB_SAMPLE_MODE, value::SAMPLE_ALL);
    if (sample_mode == value::SAMPLE_SINGLE)
    {
      initial_command_line += " --slate";
    }
